...
public class HypothyrodismEliminate extends InitClass {
	...
	String HypothyroidismEliminate_Morbidity = "HypothyroidismEliminate";
	//starting the timer
	timer_start = Instant.now();
				...
				//Navigating to HypothyroidismEliminate recipes section
				WebElement Recipes = driver.findElement(By.xpath("//div[normalize-space()='RECIPES']"));
				Recipes.click();
				WebElement HypothyroidismEliminate = driver.findElement(By.id("ctl00_cntleftpanel_ttlhealthtree_tvTtlHealtht211"));
				...
				for(int page = 1; page <= pageSize; page++ ) {	
					...
					// fetching recipe name
						...
						for (int j = 0; j < acceptedRecipeCatList.size(); j++) {
							...
							xlUtil.setCellData("HypothyroidismEliminate", PageNumber, 2, recipeCatListPresent.toString());
						}
						...
						String foodCategory = acceptedFoodCatList.get(j);
							...
							xlUtil.setCellData("HypothyroidismEliminate", PageNumber, 3, FinalFoodCategory);
						}
						// iterate through tags webElement to capture morbid condition and write in excel
						
						//check for HypothyroidismEliminate ingredient eliminate list in the recipes
						for (int j = 0; j < targetedMorbidCondList.size(); j++) {
							...
							if (tagsText.contains(tarMorbidCondition)) {
								...
								xlUtil.setCellData("HypothyroidismEliminate", PageNumber, 9,
						...
						xlUtil.setCellData("HypothyroidismEliminate", PageNumber, 10, URLString);
						xlUtil.setCellData("HypothyroidismEliminate", PageNumber, 0, recipe_id);
						xlUtil.setCellData("HypothyroidismEliminate", PageNumber, 1, recipe_name);
						xlUtil.setCellData("HypothyroidismEliminate", PageNumber, 4, ingredients);
						xlUtil.setCellData("HypothyroidismEliminate", PageNumber, 5, prep_Time);
						xlUtil.setCellData("HypothyroidismEliminate", PageNumber, 6, cook_Time);
						xlUtil.setCellData("HypothyroidismEliminate", PageNumber, 7, method);
						//xlUtil.setCellData("HypothyroidismEliminate", PageNumber, 3, FinalFoodCategory);
