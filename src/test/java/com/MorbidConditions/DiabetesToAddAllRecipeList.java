...
public class DiabetesToAddAllRecipeList extends InitClass {
	...
	public void scrapeDiabeticToAddRecipe() throws InterruptedException, IOException {
		...
		// creating Excel
				...
				excelReader.createExcel("Diabetes_ToAddAllrecipe");
		...
		List<WebElement> paginationList = driver
				.findElements(By.xpath("//div[@style='text-align:right;padding-bottom:15px;']/a"));
		...
		for (int page = 1; page <= pageSize; page++) {
			...
			WebElement pagination = driver
					.findElement(By.xpath("//div[@style='text-align:right;padding-bottom:15px;']/a[" + page + "]"));
			pagination.click();
			Thread.sleep(1000);
...
//				// check3 write only if morbid condition diabetic present
//				if (!tagstext.contains(categoryList.morbidCondition)) {
//
//					System.out.println(
//							"navigate back--> Target Morbid condition not present--*****************************************************************");
//					driver.navigate().to("https://www.tarladalal.com/RecipeAtoZ.aspx?pageindex=" + page);
//					continue;
//
//				}
				...
				excelReader.setCellData("Diabetes_ToAddAllrecipe", cell, 11, toAddIngredientName);
				...
				excelReader.setCellData("Diabetes_ToAddAllrecipe", cell, 0, recipeId);
				excelReader.setCellData("Diabetes_ToAddAllrecipe", cell, 1, recipeName);
				...
				for (int j = 0; j < acceptedRecipeCatList.size(); j++) {
					...
					excelReader.setCellData("Diabetes_ToAddAllrecipe", cell, 2, recipeCatListPresent.toString());
				}
				...
				for (int j = 0; j < acceptedFoodCatList.size(); j++) {
					...
					excelReader.setCellData("Diabetes_ToAddAllrecipe", cell, 3, foodCatListPresent.toString());
				}
				
				excelReader.setCellData("Diabetes_ToAddAllrecipe", cell, 4, ingredients);
				excelReader.setCellData("Diabetes_ToAddAllrecipe", cell, 5, prepartionTime);
				excelReader.setCellData("Diabetes_ToAddAllrecipe", cell, 6, cookingTime);
				excelReader.setCellData("Diabetes_ToAddAllrecipe", cell, 7, preparationMethod);
				excelReader.setCellData("Diabetes_ToAddAllrecipe", cell, 8, nutrient);
				...
				for (int j = 0; j < targetedMorbidCondList.size(); j++) {
					...
					if (tagstext.contains(tarMorbidCondition)) {
						...
						excelReader.setCellData("Diabetes_ToAddAllrecipe", cell, 9,
								morbidCondListPresent.toString());
					}
				}
				excelReader.setCellData("Diabetes_ToAddAllrecipe", cell, 10, recipeUrl);
